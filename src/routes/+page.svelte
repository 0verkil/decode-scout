<script lang="ts">
  ...
  import { getArtifactStats, getMeanTimeBetweenEventTypes } from "$lib/stats";
    ...
    // *** NEW FUNCTION FOR CLASSIFY ALL ***
    function handleClassifyAll() {
        // Iterate over a copy of the array to resolve all shots
        // We use a copy of shotsInAir since handleResolution updates it reactively
        const shotsToResolve = [...shotsInAir];
        
        shotsToResolve.forEach(shot => {
            handleResolution('classify', shot);
        });
        
        // shotsInAir should now be empty after all calls to handleResolution
    }
    // ***********************************
    ...
    <div class="flex-1 grid grid-cols-2 min-h-0 divide-x divide-zinc-800">
      ...
      <div class="flex flex-col h-full bg-black">
          ...
          <div class="flex-1 p-3 flex flex-col gap-3 bg-zinc-950 relative overflow-y-hidden">
              ...
              <button
                  class="w-full bg-blue-700 hover:bg-blue-600 active:scale-[0.98] transition-all text-white font-black text-sm uppercase rounded p-2 shadow-xl disabled:opacity-30 disabled:pointer-events-none"
                  disabled={shotsInAir.length === 0}
                  on:click={handleClassifyAll}
              >
                  CLASSIFY ALL ({shotsInAir.length} Artifacts) (+3 Each)
              </button>
